...
public class VideoDialog extends JDialog implements WindowObject<Video> {
	...
	private void initialize() {
		...
		tabbedPane.addTab("Ogolny", null, generalPanel, null);
		generalPanel.setLayout(new GridLayout(0, 2, 0, 0));

		JLabel label = new JLabel("Tytul");
		generalPanel.add(label);

		generalPanel.add(titleText);

		JLabel label2 = new JLabel("Cena za dzien");
		generalPanel.add(label2);

		generalPanel.add(perDayText);

		JLabel label_1 = new JLabel("Plyty");
		...
		tabbedPane.addTab("Wpisy dodatkowe", null, new VideoEntryPanel(frame, model),
				null);
